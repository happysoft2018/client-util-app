...
### v1.3.9 - 비대화식 CLI 지원 (2025-10-29) ✨

**신규 기능:**
- 메뉴 없이 `--mode` 플래그로 직접 실행 가능
  - 모드: `db`, `telnet`, `sql`, `csv`, `config`
  - Node 실행 및 패키지 EXE 모두 지원

**문서:**
- 비대화식 CLI 사용법과 예제를 사용자 매뉴얼에 반영
- 패키지 빌드에서 APP_ROOT 상대 경로 동작 주의사항 명시
...
**버그 수정:**
- **줄바꿈 처리 개선**: 프로시저 정의 등 여러 줄 텍스트가 CSV에서 깔끔하게 표시
...
**추가 모듈 다국어 지원:**
- DBConnectionChecker, DBExecutor, TelnetChecker 모듈에 다국어 메시지 추가
- 모든 사용자 메시지가 영어/한국어로 통일
- 데이터베이스 연결 오류, SQL 실행 오류 등 모든 오류 메시지 번역
- 사용자 입력 프롬프트의 번역 품질 향상
...
### v1.3.4 - 다국어 및 인코딩 지원 (2025-10-18) 🌏

**다국어 지원 (영어/한국어):**
- UI 언어 선택을 위한 `--lang` 파라미터 추가
  - 영어: `run.bat` 실행 또는 `node app.js --lang=en`
  - 한국어: `실행하기.bat` 실행 또는 `node app.js --lang=kr`
- 모든 UI 메시지 (메뉴, 프롬프트, 오류) 두 언어로 제공
- 60개 이상의 메시지가 번역되어 원활한 사용 경험

**CSV 인코딩:**
- **모든 CSV 파일은 UTF-8 인코딩 필수**
- 한글 깨짐 문제 해결
- 모든 CSV 입력에 적용: DB 연결 체크, 텔넷 체크, SQL 파라미터
- 📝 **중요**: CSV 파일을 UTF-8 인코딩으로 저장하세요 (자세한 방법은 사용자 매뉴얼 참조)

**릴리스 스크립트 개선:**
- `package.json`에서 버전 자동 읽기
- 영어/한국어 실행 스크립트 자동 생성
- 배포용 ZIP 아카이브 자동 생성
